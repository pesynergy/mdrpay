...
class Permission {
    ...
    public static function commission($id)
    {
        ...
        if($report->product == "collection"){
            $provider = \App\Model\Provider::where('recharge1', 'payin_charge')->first()->id;
        }else{
            $provider = $report->provider_id;
        }
        
        $parent   = User::where('id', $report->user->parent_id)->first(['id', 'payoutwallet', 'scheme_id', 'role_id', 'parent_id']);
    ...
    public static function curl($url , $method='GET', $parameters, $header, $log="no", $modal="none", $txnid="none")
    {
        ...
        if($log != "no"){
            \DB::table("log_apis")->insert([
                ...
                "modal" => $modal,
                "txnid" => $txnid,
                "header"   => urlencode(json_encode($header)),
                "request"  => urlencode(json_encode($parameters)),
                "response" => urlencode(json_encode($code."/".$err."/".$response))
            ]);
        }
