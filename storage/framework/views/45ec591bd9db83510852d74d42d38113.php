    ...
    <div id="permissionModal" class="modal fade" tabindex="-1" aria-labelledby="permissionModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
            <div class="modal-content">
                ...
                <!-- Header -->
                <div class="modal-header bg-slate text-white">
                    <h6 class="modal-title" id="permissionModalLabel">Member Permission</h6>
                    ...
                    <div class="modal-body p-0">
                        <table class="table table-hover table-bordered mb-0">
                            <thead class="table-light">
                                <tr>
                                    <th width="200px;">Section Category</th>
                                    <th>
...
var onDraw = function() {
    $('button.toggle-status').off('click').on('click', function(evt){
        ...
        var ele = $(this);
        var id = ele.data('id');
        var currentStatus = ele.text().trim();
        ...
        .done(function(data) {
            if(data.status == "success"){
                // Update button text instantly without reload
                ele.text((newStatus === "active") ? "Active" : "Inactive");
        ...
        .fail(function(errors) {
            notify("Request failed. Try again.", 'warning');
        });
    });
};
        ...
        var options = [
            {
                ...
                render: function(data, type, full, meta){
                    var statusText = (full.status == "active") ? "Active" : "Inactive";

                    // Always dark button
                    return `<button class="btn btn-xs btn-dark toggle-status" data-id="` + full.id + `">` + statusText + `</button>`;
                }
            },
            { "data": "parents"},
            { "data" : "agentcode",
            ...
            { "data" : "email",
                render: function(data, type, full, meta) {
                        return `Email : ` + full.email + `<br>Mobile no : ` + full.mobile;
                    }
            },
            { "data" : "mainwallet",
                render: function(data, type, full, meta) {
                        return `Collection : ` + full.mainwallet + `<br>Payout : ` + full.payoutwallet + `<br>Locked : ` + full.lockedamount;
                    }
            },
            { "data" : "agentcode",
                render:function(data, type, full, meta){
                    ...
                    out +=  `<ul class="icons-list">
                                <li class="dropdown">
                                    <a href="#" class="dropdown-toggle mt-10 btn btn-dark btn-xs btn-labeled legitRipple" data-toggle="dropdown">
